...
class PrepNotes {
   ...
   */
  async getPrepNotes(filter) {
    let query = `${API_BASE_URL}/api/PrepNotes/`;
    if (filter) {
  ...
  /**
   * Update certain values on a PrepNotes record. Must send in an id
   * @param {string|number} id required
   * @param {JSON} updates required
   */
  async updatePrepNotes(id, updates) {
    if (!id) {
